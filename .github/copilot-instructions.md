...
When generating a new Umbraco package project or example site:

- Always ensure `appsettings.Development.json` contains an `Umbraco:CMS:Unattended` configuration section.
- Use unattended install settings for local development.
- Do not overwrite existing `Umbraco:CMS` configuration sections; merge the `Unattended` section into the existing structure.
- The unattended credentials are intentionally hardcoded because they are only used for local development with a local SQLite database.
...
"Umbraco": {
  "CMS": {
    "Unattended": {
      ...
      "UnattendedUserName": "admin@admin.com",
...
* Support repeatable local development setup using unattended Umbraco installs
